                    ...
                    while (iter.hasNext()) {
                        ...
                        if (key.isAcceptable()) { // 接收请求
                            ...
                            if (request.Path.equals("/favicon.ico")
                                    ...
                                    || request.Path.toLowerCase().indexOf("/statics") == 0) {
                                // ##处理
                                // #说明是访问静态文件
                            ...
                            if (!accessStaticFile) {
                                ...
                                // 获取客户端IP
                                request.setRemoteIP(
                                        channel.socket().getLocalAddress().getHostName());
                                // 开始分配到Handler
                                ...
                                ByteBuffer writerBuffer = ByteBuffer
                                        .allocate(headerBytes.length + contentBytes.length);
                                writerBuffer.clear();
